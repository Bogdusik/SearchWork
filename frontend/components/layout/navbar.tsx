...
export function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'fixed top-14 inset-x-0 bottom-0 z-50 bg-[#030303]/95 backdrop-blur-xl border-t border-white/[0.06] md:hidden overflow-hidden',
          open ? 'flex flex-col' : 'hidden',
        )}
      >
        <div
          data-slot={open ? 'open' : 'closed'}
          className="data-[slot=open]:animate-in data-[slot=open]:fade-in data-[slot=open]:zoom-in-95 data-[slot=closed]:animate-out data-[slot=closed]:fade-out data-[slot=closed]:zoom-out-95 ease-out duration-200 flex flex-col p-6 gap-1"
        >
          {links.map((link) => (
            <Link
              key={link.href}
