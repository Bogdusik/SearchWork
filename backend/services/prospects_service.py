...
RSS_URLS = [
    "https://www.prospects.ac.uk/jobs.xml",
    "https://www.prospects.ac.uk/jobs/rss/",
    "https://www.prospects.ac.uk/jobs/feed/",
]
...
def _parse_rss(content: bytes, query: str, results_per_page: int) -> list[dict]:
    ...
    for item in channel.findall("item"):
        ...
        if words and not any(w in title_lower or w in desc_lower for w in words):
            continue
        ...
        for tag in item:
            ...
            if "company" in tag_name or "employer" in tag_name or "organisation" in tag_name:
        ...
        jobs.append({
            "external_id": f"prospects_{external_id}",
            "source": "prospects",
            "title": title,
            "company": company,
